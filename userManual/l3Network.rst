...
网络服务供应商（network service providers）提供的网络服务, 通常是OSI三层到七层协议的软件实现.
...
为了给L3网络上的虚拟机提供服务，网络服务实现了OSI三层到七层的协议. 和L3网络的父L2网络相关联的网络服务提供者负责提供网络服务. 单个类型的网络服务可以有多个网络服务提供者, 同时单个网络服务提供者可以提供多种类型的服务.
用户可以在创建的L3网络上挂载网络服务并选择网络服务提供者. 在ZStack当前版本中, 支持的服务/网络服务提供者如下所示:
...
表中, 可以挂载的L2网络列表明网络服务提供者可以挂载到哪种L2网络. 网络服务提供者不能为不可以挂载的L2网络的子L3网络提供服务.
...
系统L3网络是ZStack保留使用的，用户不能使用它创建虚拟机. 系统L3网络通畅被公共网络（public networks）和管理网络（management networks）使用. 
通畅云中的用户虚拟机不应该有网卡连接在公网或管理网络上, 但设备虚拟机（appliance VMs） (例如路由器虚拟机，router
VM)确实需要有网卡连接在这些网络上; 因此系统网络和公共网络可以创建为系统L3网络.

.. 注意:: 管理网络和公共网络可以被创建为非系统L3网络, 这样将允许用户虚拟机使用这些网络.
...
.. list-table::
   ...
   * - **l3NetworkUuid**
     ...
     - 0.6
   * - **networkServices**
     - 网络服务提供者UUID和网络服务类型列表之前的映射关系 (map)
     -
     -
     - 0.6

.. 注意:: 可以使用QueryNetworkServiceProvider来查询一个网络服务提供者的UUID, 例如::

              QueryNetworkServiceProvider fields=uuid name=VirtualRouter

          如果你想查看一个网络服务提供者提供的所有网络服务, 你可以去掉'field'参数, 例如::
