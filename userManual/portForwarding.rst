...
该配置决定Whether to source NAT inbound traffic of a port forwarding rule. 如果设置为true, 到达portForwardingRule.guestIp的流量会拥有portForwardingRule.vipIp作为源IP（source IP）; 这在一个虚拟机上挂载了多个端口转发规则的时候比较有用; 它会强制虚拟机通过VIP回复流入的流量至数据包的来源, 而不是通过默认路由来回复.
